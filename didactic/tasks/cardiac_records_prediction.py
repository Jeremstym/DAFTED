...
class CardiacRecordsPredictionTask:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tabular_attrs = list(TabularAttribute[e] for e in tabular_attrs)
        ...
        if use_time_series:
            ...
            self.views = list(ViewEnum[e] for e in views)
            self.time_series_attrs = list(TimeSeriesAttribute[e] for e in time_series_attrs)
    ...
    def _prepare_data_subset(
        self, data: CardinalDataModule, subset: str, use_time_series: bool = False
        ...
        print(f"Setting up data for subset: {subset}")
        match subset:
        ...
        if subset == "test":
            ...
            # data_list, _, _ = dataloader
            # dataloader = data_list["query"]
            dataloader = dataloader.iterables["query"]

        target_attr = "diagnosis"
        if target_attr in self.tabular_attrs:
            self.tabular_attrs.remove(target_attr)

        # For each tabular feature, save the vectors of values over each batch
        tab_data: Dict[TabularAttribute, List[np.ndarray]] = {}
        for batch, attr in itertools.product(dataloader, [*self.tabular_attrs, target_attr]):
        ...
        target = tab_data_stacked.pop(target_attr)
        ...
        if use_time_series:
            time_series_attrs = self.time_series_attrs
            views = self.views
            ts_data: Dict[Tuple[str, str], List[Tensor]] = {}
            for batch in dataloader:
                ts_batch_data = filter_time_series_attributes(batch, views, time_series_attrs)[
                    0
                ...
                for cross_attrs in ts_batch_data:
                    ...
                    attr_data = (
                        np.expand_dims(ts_batch_data[cross_attrs], axis=0)
                        if len(ts_batch_data[cross_attrs].shape) == 1
                        else ts_batch_data[cross_attrs]
                    )
                    ts_data_value = F.interpolate(
                        torch.from_numpy(
                            np.expand_dims(attr_data, axis=1),
                        ),
                        size=64,
                        mode="linear",
                    ).squeeze(dim=1)
                    print(f"time series data shape for {cross_attrs_column}: {ts_data_value.shape}")
                    ts_data.setdefault(cross_attrs_column, []).append(ts_data_value.numpy())
            ...
            tab_df = ts_df # Use only time series data for this task

        print(f"tab_df shape {tab_df.shape}")
