...
class CardiacSequenceAttributesPCA:
    ...
    pca: Union[PCA, Dict[Tuple[ViewEnum, TimeSeriesAttribute], PCA]]
    pca_instance: PCA
    _attrs_stats: Dict[Tuple[ViewEnum, TimeSeriesAttribute], Tuple[float | np.ndarray, float | np.ndarray]]
    ...
    def in_shape(self) -> Tuple[int, int]:
        ...
        match self.strategy:
            case "global_pca":
                self.pca_instance = self.pca
            case "attr_pca":
                self.pca_instance = next(iter(self.pca.values()))
        return 1, self.pca_instance.n_features_in_
    ...
    def preprocess_attr_data(
        cls,
        data: np.ndarray,
        bounds: Optional[Tuple[float | np.ndarray, float | np.ndarray]] = None,
    ...
    def __call__(
        ...
        match self.strategy:
            ...
            case "attr_pca":
                assert attr is not None, "Attribute must be specified when using 'attr_pca' strategy."
                self.pca_instance = self.pca[attr]
        ...
        if out_shape is None:
            ...
            if task == "decode":
                ...
                out_shape = (self.pca_instance.n_features_in_,)
            else:
        ...
        if task in ["encode", "reconstruct"]:
            ...
            x = self.preprocess_attr_data(x, bounds=attr_bounds, length=self.pca_instance.n_features_in_ // x.shape[1])
            x = self.pca_instance.transform(x.reshape(len(x), self.pca_instance.n_features_in_))
        if task in ["decode", "reconstruct"]:
            x = self.pca_instance.inverse_transform(x).reshape(len(x), max(out_channels, 1), -1)
    ...
    def fit(self, samples: Dict[Tuple[ViewEnum, TimeSeriesAttribute], np.ndarray]) -> "CardiacSequenceAttributesPCA":
        ...
        match self.strategy:
            case "global_pca":
                ...
                stack_samples = np.vstack(
                    [
                        minmax_scaling(attr_samples, self._attrs_stats[attr_key])
                        for attr_key, attr_samples in samples.items()
                    ]
                )
                self.pca = PCA(n_components=self._n_components, **self._pca_kwargs).fit(stack_samples)
            case "attr_pca":
