...
from typing import Dict, Literal, Optional, Sequence, Tuple, TypeVar, Union
...
class CardiacSequenceAttributesAutoencoder(SharedStepsTask):
    ...
    def in_shape(self) -> Tuple[int, ...]:
        ...
        return tuple(self.example_input_array.shape[1:])
    ...
    def on_fit_start(self) -> None:
        ...
        attrs_stats_update_fn = {
            _AttributeStatistic.min: lambda cur_stat, attr_data: min(cur_stat, attr_data.min()),
            _AttributeStatistic.max: lambda cur_stat, attr_data: max(cur_stat, attr_data.max()),
        }
        train_dl = self.trainer.datamodule.train_dataloader()  # type: ignore[attr-defined]
        for batch in train_dl:
            for (stat, update_fn), (attr, attr_data) in itertools.product(
                attrs_stats_update_fn.items(),
                filter_time_series_attributes(batch, views=self.hparams["views"], attrs=self.hparams["attrs"])[
                    0
                ].items(),
            ):
    ...
    def preprocess_attr_data(
        cls, data: T, bounds: Optional[Tuple[Tensor, Tensor]] = None, length: Optional[int] = None
    ) -> T:
        ...
        if bounds is not None:
            ...
            data = minmax_scaling(data, bounds=bounds) # type: ignore[arg-type]
        return data

    @classmethod
    def postprocess_attr_prediction(
        cls, prediction: Tensor, bounds: Optional[Tuple[Tensor, Tensor]] = None, length: Optional[int] = None
    ) -> Tensor:
    ...
    def forward(
        ...
        attr: Optional[Tuple[ViewEnum, TimeSeriesAttribute]] = None,
    ...
    def _shared_step(self, batch: PatientData, batch_idx: int) -> Dict[str, Tensor]:  # noqa: D102
        attrs, _ = filter_time_series_attributes(batch, views=self.hparams["views"], attrs=self.hparams["attrs"])
        ...
        for attr_key, attr_data in attrs.items():
            attrs_z[attr_key] = self.encoder(attr_data.unsqueeze(1))  # type: ignore[attr-defined]
            attrs_x_hat[attr_key] = self.decoder(attrs_z[attr_key]).squeeze(dim=1)

        # Compute input reconstruction metrics
        attrs_rec_losses = {
            attr_key: self.reconstruction_loss(attrs_x_hat[attr_key], attr) for attr_key, attr in attrs.items()  # type: ignore[operator]
        }
        metrics = {
            "/".join((self._reconstruction_loss_name, *attr_key)): self.reconstruction_loss(attrs_x_hat[attr_key], attr)  # type: ignore[operator]
    ...
    def _normalize_attrs(
        self, attrs: Dict[Tuple[ViewEnum, TimeSeriesAttribute], T]
    ...
    def predict_step(  # noqa: D102
        ...
        for attr_key, attr_data in attrs.items():
            if not (is_batch := attr_data.ndim == 2):
                attr_data = attr_data.unsqueeze(0)  # type: ignore[attr-defined]
...
class CardiacSequenceAttributesAutoencoderTokenizer(nn.Module):
    ...
    def __init__(
        self,
        cardiac_sequence_attrs_model: Union[str | Path | CardiacSequenceAttributesAutoencoder],
    ):
        """Initializes class instance.
