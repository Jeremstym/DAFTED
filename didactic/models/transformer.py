...
from didactic.models.baselines import BidirectionalMultimodalAttention
from didactic.models.layers import (
    MultiheadAttention,
    MultiheadCrossAttention,
    MultiheadDiffAttention,
    MultiheadDiffCrossAttention,
    get_nn_module,
)
...
def aggregate_tokens(
    ts_tokens: Tensor, tab_unique_tokens: Tensor, tab_shared_tokens: Tensor, mode: str = "average"
    ...
    if mode == "average":
        ...
        return (
            ts_sequence_pooling(ts_tokens),
            tab_unique_pooling(tab_unique_tokens),
            tab_shared_pooling(tab_shared_tokens),
        )
    else:
...
class FT_Transformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        attention_initialization: Literal["kaiming", "xavier"],
        ...
        assert not (
            n_cross_blocks and n_bidirectional_blocks
        ), "Cannot use both cross-attention and bidirectional attention blocks"
        ...
        if self.n_self_blocks:
            layers += [self._init_attention_block(layer_idx) for layer_idx in range(self.n_self_blocks)]
        ...
        if self.n_cross_blocks:
            ...
            layers += [
                ...
                for layer_idx in range(
                    self.n_self_blocks + self.n_cross_blocks,
                    self.n_self_blocks + self.n_cross_blocks + self.n_bidirectional_blocks,
                )
            ]
    ...
    def _init_bidirectional_block(self, layer_idx: int) -> nn.ModuleDict:
        ...
        for modality_side in ["l", "r"]:
            ...
            if layer_idx or not self.prenormalization or self.first_prenormalization:
                layer[f"{modality_side}_bidirectional_attention_normalization"] = get_nn_module(
    ...
    def _start_residual(
        self, layer: nn.ModuleDict, layer_name: str, x: Tensor, stage: str = "self_attention"
    ) -> Tensor:
        match stage:
    ...
    def _end_residual(
        ...
        layer_name: str,
        x: Tensor,
        x_residual: Optional[Tensor] = None,
        stage: str = "self_attention",
    ) -> Tensor:
        match stage:
            case "cross_attention":
                assert layer_name in [
    ...
    def forward(self, x: Tensor, x_context: Optional[Tensor] = None) -> Tensor:
        ...
        for block in self_attention_blocks:
            ...
            if x_context is not None:
                ...
                x_context = self._end_residual(
                    block, "r_attention", x_context, x_context_residual, stage="self_attention"
                )
        ...
        for block in cross_attention_blocks:
            assert x_context is not None, "x_context must be provided for cross-attention blocks"
            block = cast(nn.ModuleDict, block)
            ...
            x_context = self._end_residual(
                block, "r_cross_attention", x_context, x_context_residual, stage="cross_attention"
            )
            ...
            x_context = self._end_residual(
                block, "r_self_attention", x_context, x_context_residual, stage="cross_attention"
            )
        ...
        for block in bidirectional_attention_blocks:
            assert x_context is not None, "x_context must be provided for bidirectional attention blocks"
            block = cast(nn.ModuleDict, block)

            # Normalize the tokens from both modalities if prenormalization is enabled
            x_residual = self._start_residual(block, "l_bidirectional_attention", x, stage="bidirectional_attention")
            x_context_residual = self._start_residual(
                block, "r_bidirectional_attention", x_context, stage="bidirectional_attention"
            )
            ...
            x_context = self._end_residual(
                block, "r_bidirectional_attention", x_context, x_context_residual, stage="bidirectional_attention"
            )
            ...
            x_context_residual = self._start_residual(
                block, "r_ffn_bidirectional", x_context, stage="bidirectional_attention"
            )
            x_context_residual = block["r_ffn_bidirectional"](x_context_residual)
            x_context = self._end_residual(
                block, "r_ffn_bidirectional", x_context, x_context_residual, stage="bidirectional_attention"
            )
...
class FT_Alignment(nn.Module):
    ...
    def forward(
        self, x: Tensor, x_context: Optional[Tensor] = None, output_intermediate: bool = False
...
class FT_Interleaved(nn.Module):
    ...
    def forward(self, x: Tensor, x_context: Tensor) -> Tensor:
        """
        Performs a forward pass through the successive transformer blocks.
        ...
        for block in self.blocks:
            if "cross_attention" not in block:
                ...
                if self.unique_cross_block:
                    ...
                    x_context_residual = self._start_residual(
                        block, "cross_attention", x_context, stage="cross_attention"
                    )
                    ...
                    x_context = self._end_residual(
                        block, "cross_attention", x_context, x_context_residual, stage="cross_attention"
                    )
                    ...
                    x_context = self._end_residual(
                        block, "cross_ffn", x_context, x_context_residual, stage="cross_attention"
                    )
...
class FT_DiffInterleaved_Alignment(nn.Module):
    """Transformer with extra features.
    ...
    def _init_attention_block(self, layer_idx: int) -> nn.ModuleDict:
        layer = nn.ModuleDict(
            {
                f"attention": MultiheadDiffAttention(
                    d_token=self.d_token,
                    n_heads=self.attention_n_heads,
                    dropout=self.attention_dropout,
                    depth=layer_idx,
    ...
    def _init_cross_attention_block(self, layer_idx: int) -> nn.ModuleDict:
        layer = nn.ModuleDict(
            {
                "cross_attention": MultiheadDiffCrossAttention(
                    d_token=self.d_token,
                    n_heads=self.attention_n_heads,
                    dropout=self.attention_dropout,
                    depth=layer_idx,
...
class FT_Interleaved_Alignment_NoCross(nn.Module):
    """Transformer with extra features.
        ...
        for layer_idx, block in enumerate(self.blocks):
            if "cross_attention" not in block:
                ...
                if self.unique_cross_block and layer_idx < self.n_self_blocks - 1:
                    ...
                    # Forward pass through the cross-attention block
                    x_residual, _ = block["cross_attention"](x_residual, x_context)
...
class FT_Interleaved_2UniFTs_nosubmodule(nn.Module):
    """Transformer with extra features.
