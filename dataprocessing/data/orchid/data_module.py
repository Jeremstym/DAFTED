...
class OrchidDataModule(VitalDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:  # noqa: D102
        ...
        # Check if dataset is not already setup
        # remaining_subsets = set(subsets_to_setup) - set(self.datasets.keys())
    ...
    def predict_dataloader(self) -> Sequence[DataLoader]:  # noqa: D102
        return [
            DataLoader(self.datasets[Subset.TEST], batch_size=None, num_workers=self.num_workers, pin_memory=True),
            DataLoader(self.datasets[Subset.TRAIN], batch_size=len(self.datasets[Subset.TRAIN]), num_workers=self.num_workers, pin_memory=True),
            DataLoader(self.datasets[Subset.VAL], batch_size=len(self.datasets[Subset.VAL]), num_workers=self.num_workers, pin_memory=True),
        ]
