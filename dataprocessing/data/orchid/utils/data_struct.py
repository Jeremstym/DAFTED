...
class Patient:
    ...
    #     **kwargs,
    # ) -> "Patient":
    #     """Loads image data and metadata files, from a processed data directory, related to one patient.

    #     Args:
    #         processed_data_dir: Root folder in which to look for files related to the patient. We look for files
    #             recursively inside the directories, so as long as the files respect the `IMG_FILENAME_PATTERN` pattern,
    #             they will be collected.
    #         subdir_levels: Levels of subdirectories under `processed_data_dir`.
    #         kwargs: Parameters to pass along to each of the views' ``from_dir`` method.
    #     Returns:
    #         `Patient` instance, built using data related to the patient from the provided processed data folder.
    #     """
    #     data_roots = [processed_data_dir]
    #     if subdir_levels:
    #         # If subdirectory levels are specified, adjust the data roots accordingly
    #         for level in subdir_levels:
    #             if level == "patient":
    #                 data_roots = [data_root / self.id for data_root in data_roots]
    #             elif level == "view":
    #                 data_roots = [
    #                     data_root / view_enum.value for data_root in data_roots for view_enum in ViewEnum
    #                 ]
    #             else:
    #                 raise ValueError(
    #                     f"Unexpected subdirectory level: {level}. Supported levels are 'patient' and 'view'."
    #                 )

    #     return Patient.from_dir(
    #         patient_id=self.id,
...
class View:
    ...
    def _init_data_and_attrs(
        self, data_tag: str, data_file: Path, overwrite_attrs_cache: bool = False, auto_pickling: bool = False
    ) -> None:
        ...
        # self.data[data_tag] = lambda: self._load_data_and_attrs(
        #     data_tag, overwrite_attrs_cache=overwrite_attrs_cache, auto_pickling=auto_pickling
        # )[0]
        self.data[data_tag] = None  # Placeholder to reserve the key in the LazyDict
        self.attrs[data_tag] = lambda: self._load_attrs(
            data_tag, overwrite_attrs_cache=overwrite_attrs_cache, auto_pickling=auto_pickling
        )

    def _load_data_and_attrs(
        self, data_tag: str, overwrite_attrs_cache: bool = False, auto_pickling: bool = False
        ...
        if attrs_cache_path.exists() and not overwrite_attrs_cache:
            if auto_pickling:
                cached_attrs = np.load(attrs_cache_path, allow_pickle=True)
                attrs_arrays = {key: np.array(cached_attrs[key]).copy() for key in cached_attrs.files}
                del cached_attrs
            else:
                attrs_arrays = np.load(attrs_cache_path)
        else:
            raise Exception(f"Attributes cache not found for '{data_tag}' at '{attrs_cache_path}'. ")

        self.add_image(data_tag, im_array, voxelspacing=voxelspacing, precomputed_attrs=attrs_arrays)
    ...
    def _load_attrs(
        ...
        if attrs_cache_path.exists() and not overwrite_attrs_cache:
            if auto_pickling:
                cached_attrs = np.load(attrs_cache_path, allow_pickle=True)
                self.attrs[data_tag] = {key: np.array(cached_attrs[key]).copy() for key in cached_attrs.files}
                del cached_attrs
            else:
    ...
    def from_dir(
        ...
        view_files_pattern = IMG_FILENAME_PATTERN.format(
            patient_id=patient_id, view=view, tag="mask", ext=as_file_extension(img_format)
        )
        # view_files = {}
        # for data_root in data_roots:
            # Search recursively inside the provided directory
        ...
        data_tag = "mask"
        view_file = Path(data_roots[0]) / patient_id / view_files_pattern
        view_object._init_data_and_attrs(
            data_tag, view_file, overwrite_attrs_cache=overwrite_attrs_cache, auto_pickling=auto_pickling
        )
        if eager_loading:
