        ...
        time_series_attr_by_group = {
            group_label: {
                patient.id: (
                    ...
                    if attr_view in patient.get_mask_attributes(mask_tag)
                    and attr in patient.get_mask_attributes(mask_tag)[attr_view]
...
def generate_patients_splits(
    ...
    """
    # First check if stratify is a valid tabular attribute
    if stratify not in TabularAttribute:
        raise ValueError(f"Invalid stratify attribute: {stratify}. Must be one of {list(TabularAttribute)}.")
    patients = patients.values()
    ...
    patients_stratify = {}
    for patient in patients:
        if stratify in patient.attrs and patient.attrs[stratify] in TABULAR_CAT_ATTR_LABELS[stratify]:
            patients_stratify[patient.id] = patient.attrs[stratify]
        else:
            # raise KeyError(f"Attribute '{stratify}' not found in patient {patient.id}.")
            logger.warning(
                f"Attribute '{stratify}' not found in patient {patient.id}. Skipping this patient for stratification."
            )
    ...
    df = pd.DataFrame.from_dict(patients_stratify, orient="index", columns=[stratify])
    # print(f"Stratify labels: {df.value_counts()}")
    # raise Exception
...
def generate_cv_splits(
    ...
    for i, (train_index, test_index) in enumerate(skf.split(list(patients_stratify), stratify_labels)):
        patient_ids_train, patient_ids_test = [list(patients_stratify)[i] for i in train_index], [
            list(patients_stratify)[i] for i in test_index
        ]
