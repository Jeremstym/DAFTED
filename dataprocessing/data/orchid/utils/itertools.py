...
def load_patient_worker(
    patient_id, data_roots, views, handle_attrs_errors, img_format, eager_loading, overwrite_attrs_cache, auto_pickling=False
):
    """
    This function is called by each worker process.
    It calls Patient.from_dir, which itself loads data files
    including any NPZ files, inside the worker.
    """
    patient = Patient.from_dir(
        patient_id,
...
class Patients(Collection, Mapping[Patient.Id, Patient]):
    ...
    def __init__(
        ...
        if views is not None:
            if len(views) == 1:
                logger.warning("Make sure that data are stratified properly when filtering by a single view.")
            #     # Exclude patients that do not have the requested view
            #     patient_ids = [
            #         patient_id
            #         for patient_id in patient_ids
            #         if views[0] in views_avail_by_patient(data_roots, patient_id)
            #     ]
            # elif len(views) == 2:
            #     # Exclude patients that do not have the requested views
            #     patient_ids = [
            #         patient_id
            #         for patient_id in patient_ids
            #         if all(view in views_avail_by_patient(data_roots, patient_id) for view in views)
            #     ]
        ...
        def load_patient_worker_wrapper(
            pid, data_roots, views, handle_attrs_errors, img_format, eager_loading, overwrite_attrs_cache
        ):
            return load_patient_worker(
                pid, data_roots, views, handle_attrs_errors, img_format, eager_loading, overwrite_attrs_cache
            )
        ...
        if use_multiprocessing:
            ...
            with Pool(processes=8, maxtasksperchild=1) as pool:
                ...
                results_iterator = pool.imap(partial_worker, patient_ids)
                results = list(tqdm(results_iterator, total=len(patient_ids), desc="Loading patients"))
            # Convert to dictionary
            self._patients = dict(results)
            # Clean up memory (ENFORCE)
            pool.close()
            pool.join()
            del pool
            gc.collect()
