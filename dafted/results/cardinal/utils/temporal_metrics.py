...
class TemporalMetrics(Metrics, TimeSeriesAttributesMixin):
    ...
    def __init__(
        ...
        thresholds_cfg: Optional[Union[str, Path]] = None,
        threshold_margins: Optional[Mapping[str, float]] = None,
    ...
    def process_result(self, result: View | Patient) -> Optional[Tuple[str, "TemporalMetrics.ProcessingOutput"]]:
        ...
        attrs_to_evaluate = {
            attr: attr_vals
            for attr, attr_vals in self._extract_attributes_from_result(result, self.input_tag).items() # type: ignore[arg-type]
    ...
    def _aggregate_metrics(self, metrics: pd.DataFrame) -> pd.DataFrame:
        ...
        if self._thresholds:
            ...
            frames_ratio_agg_metrics = metrics.loc[temporally_inconsistent_indices, frames_ratio_cols].agg( #type: ignore[index]
                ["mean", "std", "max", "min"]
            )
