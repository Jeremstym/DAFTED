...
from typing import Any, Dict, Iterable, Literal, Sequence, Tuple, Optional, Union, List, Callable
...
from dafted.models.explain import attention_rollout, k_number, register_attn_weights_hook
from dafted.tasks.cardiac_multimodal_representation import CardiacMultimodalRepresentationTask
    ...
    **forward_kwargs,
    ...
    patients_encodings = [
        encode_patients_attrs(
            model,
            {attr: patient_attrs[attr] for attr in tab_attrs},  # type: ignore[misc]
            {(view, attr): patient_attrs[view][attr] for view in model.hparams["views"] for attr in time_series_attrs},  # type: ignore[index]
            **forward_kwargs,
        )
        for patient_attrs in patients_attrs_bar
    ]
    ...
    output_encoding = torch_to_numpy(default_collate(patients_encodings))

    return output_encoding  # type: ignore[return-value]
...
def encode_patients_attrs(
    ...
    **forward_kwargs,
    ...
    return torch_to_numpy(out_features)  # type: ignore[return-value]
...
def summarize_patient_attn(
    ...
    Args:
        ...
        attention_rollout_kwargs: When using attention rollout (`use_attention_rollout` is True), parameters to forward
            to `dafted.models.explain.attention_rollout`.
    ...
    if use_attention_rollout:
        ...
        k_numbers_reduce_fn: Callable[[List[int]], int]
        match head_reduction:
            case "mean":
                layers_attn_dict = {layer_name: layer_attn.mean(dim=0) for layer_name, layer_attn in attn_maps.items()}
                k_numbers_reduce_fn = mean  # type: ignore[assignment]
            case "k_max":
                layers_attn_dict = {
                    layer_name: layer_attn[np.argmax(k_numbers[layer_name])]  # type: ignore[index]
                    for layer_name, layer_attn in attn_maps.items()
                }
                k_numbers_reduce_fn = max
            case "k_min":
                layers_attn_dict = {
                    layer_name: layer_attn[np.argmin(k_numbers[layer_name])]  # type: ignore[index]
                    for layer_name, layer_attn in attn_maps.items()
                }
        ...
        layers_attn = torch.stack(list(layers_attn_dict.values()))
...
def summarize_patients_attn(
    model: CardiacMultimodalRepresentationTask,
    patients: Patients,
    subsets: Optional[Dict[str, Sequence[str]]] = None,
    **summarize_patient_attn_kwargs,
    ...
    patient_bar = tqdm(patients, desc=msg, unit="patient")

    patients_attn = {
        patient.id: summarize_patient_attn(model, patient, **summarize_patient_attn_kwargs) for patient in patient_bar
    }
...
def stack_time_series_attributes(
    time_series_attrs: Dict[Tuple[ViewEnum, TimeSeriesAttribute], List[np.ndarray]], view_params: Dict[str, Any]
) -> Dict[Tuple[ViewEnum, TimeSeriesAttribute], np.ndarray]:
    """Stacks lists of time-series attributes into arrays.

    Args:
        time_series_attrs: (K: S, V: List([L])), Sequence of lists of time-series attributes, where L is the
            dimensionality of each attribute.
        view_params: Parameters related to each view.
    Returns:
        ts_tokens: (N, 1, S_ts * num_time_units), Stacked time-series attributes.
    """
    ...
    for cross_attrs in time_series_attrs:
        ...
        ts_data_value = F.interpolate(time_series_attrs[cross_attrs].unsqueeze(1), size=64, mode="linear").squeeze(1)
...
def z_scoring(
    ...
    if data_support.ndim == 3:
        # data: [Samples, Batch, Features]
        # We compute mean and std on the sample dimension (dim=1)
        # Transpose
        data_support = data_support.transpose(0, 1)  # [Batch, Samples, Features]
        data_query = data_query.transpose(0, 1)  # [Batch, Samples, Features]
        ...
        # Transpose back
        data_support_z = data_support_z.transpose(0, 1)  # [Samples, Batch, Features]
        data_query_z = data_query_z.transpose(0, 1)  # [Samples, Batch, Features]
