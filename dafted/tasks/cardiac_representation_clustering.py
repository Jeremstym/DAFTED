...
class GridSearchEnsembleClustering:
    ...
    def __init__(self, num_sweeps: int, param_grid: Dict[str, Any]):
        ...
        self.sweeps_results: Optional[pd.DataFrame]
        self.gmm: GaussianMixture
        self.clusters_map = None

    def fit(self, X: np.ndarray, order_clusters_by: np.ndarray) -> "GridSearchEnsembleClustering":
        ...
        best_params = dict(params_rank.index[int(params_rank.argmax())])
...
def save_predictions(
    model: GridSearchEnsembleClustering, X: np.ndarray, save_path: Path, sample_ids: Optional[Sequence[str]] = None
) -> None:
...
def main():
    ...
    from dafted.tasks.utils import encode_patients
